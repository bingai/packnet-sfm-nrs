...
class ModelCheckpoint:
    ...
    def format_checkpoint_name(self, epoch, metrics):
        ...
        # print('0000000 filename =', filename)
        for tmp in re.findall(r'(\{.*?)[:\}]', self.filename):
            # print('0000000 tmp =', tmp)
            name = tmp[1:]
            # print('0000000 name =', name)
            filename = filename.replace(tmp, name + '={' + name)
            # print('1111111 filename =', filename)
            if name not in metrics:
                metrics[name] = 0
        filename = filename.format(**metrics)
        # print('2222222 filename =', filename)
        # print('33333 = ', os.path.join(self.dirpath, '{}.ckpt'.format(filename)))
        return os.path.join(self.dirpath, '{}.ckpt'.format(filename))

    def check_and_save(self, model, metrics):
        # Check saving interval
        # print('1---------- check_and_save-----------', metrics)
        epoch = model.current_epoch
        ...
        # print('0---------- filepath = ', filepath)
        while os.path.isfile(filepath):
            filepath = self.format_checkpoint_name(epoch, metrics)
            # print('1---------- filepath = ',filepath)
        # Check if saving or not
        if self.save_top_k != -1:
            # print('2---------- save_top_k = ', self.save_top_k)
            current = metrics.get(self.monitor)
            # print('xxxxxxxxxxxxxx', current)
