...
# rear camera intrinsic: K
def dummy_calibration(image):
    w, h = [float(d) for d in image.size]
    return np.array([[3.4420129478803830e+02, 0., 6.3498704606897275e+02],
                     [0., 3.4379271340962833e+02, 3.9379005715465081e+02],
                     [0., 0., 1]])
...
class ImageDataset(Dataset):
    ...
    def _read_rgb_context_files(self, session, filename):
        context_paths = self._get_context_file_paths(filename)
        # print('++++++++ context_paths = ', context_paths)
